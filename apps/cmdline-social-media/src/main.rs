...
async fn main() -> ReplResult<()> {
    ...
    let mut repl = Repl::new(app.clone())
        ...
        .with_command(
            ...
            FamilyApp::create_new_device,
        )
        .with_command_async(
            Command::new("create_linked_device")
                .arg(Arg::new("idkey").required(true)),
            |args, context| {
                Box::pin(FamilyApp::create_linked_device(args, context))
            },
        )
        ...
        .with_command(Command::new("get_data"), FamilyApp::get_data)
        ...
        .with_command_async(Command::new("add_datum"), |_, context| {
            Box::pin(FamilyApp::add_datum(context))
        })
        .with_command_async(
            Command::new("share")
